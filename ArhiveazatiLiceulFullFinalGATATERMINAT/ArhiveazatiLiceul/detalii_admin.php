...
function disableselect(e){
 ...
 <?php echo $_SESSION["ANAME"]; ?></h3><br><hr><br>
			<div id="section">
				...
				<script>
					document.getElementById("blocare").onkeypress = function(e) {
    ...
    if ("></\"".indexOf(chr) >= 0)
        return false;
					...
					<form  enctype="multipart/form-data" role="form"  method="post" action="<?php echo $_SERVER["PHP_SELF"];?>">
							...
							<input type="text" maxlength="10" required class="input3" name="prel" id="blocare"  pattern="[^()/><\][\\\x22,;|]+"><br><br>
							<label>  Număr de telefon
</label><br>
							<input type="number" maxlength="10" required class="input3" name="pno" id="blocare"  pattern="[^()/><\][\\\x22,;|]+"><br><br>
							<label>  E-mail
</label><br>
							<input type="email"  class="input3" required name="mail" id="blocare"  pattern="[^()/><\][\\\x22,;|]+"><br><br>
							<label>  Adresă</label><br>
							<input type="text" required class="input" name="addr" id="blocare" pattern="[^()/><\][\\\x22;|]+"><br><br>
							 
					  
					      <label>Calificare</label><br>
					     <input type="text" name="cal" required class="input" id="blocare"  pattern="[^()/><\][\\\x22,;|]+">
					     <br><br>
					      <label>Diriginte clasă</label><br>
					    <select name="clas" required class="input3" id="blocare"  pattern="[^()/><\][\\\x22,;|]+">
						<?php
							$s="select * from class";
							$re=$db->query($s);
							if($re->num_rows>0)
